...
apiRouter.post('/signin', async (req, res) => {
    try {
        ...
        if ([email, password].map(el => el.trim()).some(el => !!!el)) {
...
apiRouter.post('/forgotpassword', async (req, res) => {
    try {
        ...
        if (account) {
            ...
            const token = jwt.sign({ email }, config.RESET_TOKEN_SECRET, { expiresIn: '1h' });
            await redisClient.set(token, email);

            // send email
            await axios.post(config.EMAILER_URL, {
...
apiRouter.patch('/resetpassword', async (req, res) => {
    ...
    if ([resetToken, password].map(el => el.trim()).some(el => !!!el)) {
        return res.status(422).json({ error: 'missing fields' });
    }
